...
async function deployTestnetScaffold() {
    ...
    const assetA = {
        [symbolA]: tokenA._address
    }
    const assetB = {
        [symbolB]: tokenB._address
    }
    const assetC = {
        [symbolC]: tokenC._address
    }
    const assetS = {
        [symbolS]: diamondAddress
    }
    Static['farms'] = {}
        ...
        for (let i = 0; i <= 24; i++) {
            ...
            switch (i % 5) {
                ...
                case (1):
                    ...
                    await tokenFarmFacet.methods.deposit(
                        poolAB, [stakeAmountA, stakeAmountB],
                        blocksToStake,
                        ADDRESSZERO,
                        0
                    ).send({ from: initUsers[user] })
                ...
                case (2):
                    ...
                    await tokenFarmFacet.methods.deposit(
                        poolAC, [stakeAmountA, stakeAmountB],
                        blocksToStake,
                        ADDRESSZERO,
                        0
                    ).send({ from: initUsers[user] })
                ...
                case (3):
                    ...
                    await tokenFarmFacet.methods.deposit(
                        poolCS, [stakeAmountA, stakeAmountB],
                        blocksToStake,
                        ADDRESSZERO,
                        0
                    ).send({ from: initUsers[user] })
